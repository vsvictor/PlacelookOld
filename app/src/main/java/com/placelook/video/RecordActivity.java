...
public class RecordActivity extends Activity {
	...
	class CameraView extends SurfaceView implements SurfaceHolder.Callback,	PreviewCallback {
		...
		public void onPreviewFrame(byte[] data, Camera camera) {
			if (yuvIplimage != null && recording) {
				...
				//OpenCVFrameConverter c = new OpenCVFrameConverter.ToIplImage();
				//final Frame f = c.convert(yuvIplimage);
				//recorder.toStream(yuvIplimage);
                try {
                    recorder.record(yuvIplimage);
                } catch (FrameRecorder.Exception e) {
                    e.printStackTrace();
                }
                log.info("Frame sended RecordActivity");
			}
		}
	}
	...
	public IplImage rotate(IplImage src, int angle) {
		IplImage img = IplImage.create(src.height(), src.width(), src.depth(),
				src.nChannels());
